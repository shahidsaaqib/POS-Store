...
async function loadProducts(){
  ...
  if($('productsTable')) renderProductsTable();
...
async function importProductsCSV(file){
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // REMOVED: delete product.strength; // Remove the temporary strength field before insertion
        ...
        if (existing) {
            ...
            // Make sure not to try to update the non-existent 'strength' column
...
async function loadDashboard(){
  ...
  (itemsData || []).forEach(item => {
    ...
    if (item.sales?.id) {
      const revenue = item.quantity * item.price;
      // Purchase Price Ú©Ùˆ Ø§Ø³ØªØ¹Ù…Ø§Ù„ Ú©Ø±ØªÛ’ ÛÙˆØ¦Û’ Cost Ú©Ø§ Ø­Ø³Ø§Ø¨ Ù„Ú¯Ø§ÛŒØ§ Ú¯ÛŒØ§
